  ...
  provideLinter() {
    return {
      ...
      lint: async (editor) => {
        ...
        //sending the path to lint with a relative oath from the project
        args.push(filePath.replace(projectDir+"/",""));

        const execOpts = { env, cwd, stream: 'both' };
        if (this.disableTimeout) {
          execOpts.timeout = Infinity;
        }
        //console.log("calling:"+execPath+" with args:"+args+" with options:"+execOpts)
